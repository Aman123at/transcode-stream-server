import { PutObjectCommand, S3Client, DeleteObjectCommand, DeleteObjectsCommand , ListObjectsV2Command , GetObjectCommand} from "@aws-sdk/client-s3";
...
class S3Service {
  ...
  async generatePresignedURL(params){
    ...
    const url = await getSignedUrl(this.s3Client, command, { expiresIn: process.env.PRESIGNED_URL_EXPIRY });
    return url
  }
  ...
  async deleteTranscode(folderPath){
    // Step 1: List all objects in the transcode ID folder
    const listParams = {
    ...
    // Step 2: Delete each listed object
    const deleteParams = {
      Bucket: process.env.OUTPUT_BUCKET_NAME,
      Delete: {
